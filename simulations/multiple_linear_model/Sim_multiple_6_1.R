...
#R CMD INSTALL DBmypackage6twov2_1.0.tar.gz #install R package
library(Rcpp)
library(RcppArmadillo)
library(parallel)
library(abind)
library(DBmypackage6twov2)
...
# num_med = 1; alpha_coef =  1; beta_coef = 0; lambda1all = 1.7; lambda2all = lambda1all;
num_med = 10; #set number of mediators
alpha_coef =  rep(0, num_med);
beta_coef = rep(0,num_med); lambda1all = 2; lambda2all = lambda1all;
# num_med = 1; alpha_coef = rep(1,num_med); beta_coef = rep(0, num_med); lambda1all = 2; lambda2all = lambda1all;

########################################################
#test the product statistic
all_res_C = do.call(abind, list( mclapply(mc.cores = num_cores, all_seeds, function(mc_ind) {
  ...
  mydata_original = DBmypackage6twov2::gendata_model_X_C( n,  alpha_coef, beta_coef,  mydata_tmp, other_covariates,
                                                        coef_X_alpha, coef_X_beta, direct_effect, num_med)
  
  DBmypackage6twov2::One_MC_DB_X_noProj_C(mydata_original, other_covariates,
                                        n, omega, num_med, lambda1all, lambda2all, N_boot_out, N_boot_in)} ),
  along = 2))
...
#                                                         coef_X_alpha, coef_X_beta, direct_effect, num_med)
#   
#   DBmypackage6twov2::One_MC_DB_max_X_noProj_C(mydata_original, other_covariates, n, omega,
#                                             num_med, lambda1all, lambda2all, N_boot_out, N_boot_in)} ),
#   along = 2))

########################################################
# fig_folder = "set2/"
# figtext = paste0(fig_folder,"qq_plot.pdf")
# pdf(file=figtext)
...
tmp_pval_asym_prod = pnorm( all_res_C[(N_boot_out+7),  ] ) #sobel statistic
